...
/**
 * Cross-domain MF recommender with centroid distance regularization based on
 * fast ALS.
 *
 * @author Ignacio Fernández (ignacio.fernandezt@uam.es)
 * @author Iván Cantador (ivan.cantador@uam.es)
 */
public class CentroidMF extends FastMF {
    ...
    /**
     * Create a new instance of the Fast ALS cross-domain centroid distance MF.
     *
     ...
     * difference).
     */
    public CentroidMF(PreferenceData train, ItemNeighborhoods neighborhoods, Set<String> targetItems) {
        super(train);
                    ...
                    .mapToDouble(tgtItem -> {
                        ...
                        if (neighs != null) {
                            ...
                            for (ScoredItem ni : neighs) {
                                ...
                                float s = ni.getScore();
                                MatrixUtils.add(aux, itemFactors[j], s);
                            }
                            itemReg = MatrixUtils.distance2(itemFactors[i], aux);
                        }
    ...
    protected void itemLeastSquares() {
        ...
        // Update first source item factors, cannot be done in parallel
        sourceItems.stream()
                //.parallel()
    ...
    protected void updateTargetItem(String item, Set<String> itemUsers, float[][] G) {
        ...
        solveExtendedRR1(1, itemFactors[i], x, y, c, centroid, 1);
    }
    ...
    protected void updateSourceItem(String item, Set<String> itemUsers, float[][] G) {
        ...
        if (invNeighs != null) {
            for (ScoredItem neigh : invNeighs) {
                ...
                for (ScoredItem o : neighborhoods.neighbors(neigh.getItem())) {
                    int k = train.itemId(o.getItem());
                    if (k != j) {
                        float sk = o.getScore();
                        MatrixUtils.add(aux, itemFactors[k], -s * sk);
                    }
                }
            }
        }
        ...
        // Perform a single cycle of RR1
        solveExtendedRR1(1, itemFactors[j], x, y, c, aux, sq);
    }
    ...
    public String toString() {
        return String.format(Locale.ENGLISH, "CentroidMF_k=%d_l=%s_lc=%s_c=%s_n=%d",
